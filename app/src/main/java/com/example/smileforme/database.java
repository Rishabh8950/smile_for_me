...
public class database extends SQLiteOpenHelper {
   ...
   public database(Context Context)
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase)//all tables admin,recipient,donor,ngo,item created
    {
        ...
        sqLiteDatabase.execSQL("Create table recipientdata(id integer primary key autoincrement, name text, DOB text, " +
                "Gender text, Email text , Contact_No text, Password text, BPL_No text ,status int) ");

        sqLiteDatabase.execSQL("Create table donordata(id integer primary key autoincrement, name text, DOB text, " +
                "Gender text, Email text, Contact_No text, Password text)");

        sqLiteDatabase.execSQL("Create table ngodata(id integer primary key autoincrement, name text, ngoid text, " +
                "email text, mobile text, password text, ngodetails text, status int ) ");

        sqLiteDatabase.execSQL("Create table itemsdata(id integer primary key autoincrement, itemtype text, " +
                "itemdetails text, itemcount int, mobile text,BPL text)");
    }
    @Override
    ...
    public int recselect( String Email, String Password)//login access to recipient
    {
        ...
        Cursor cursor=this.getReadableDatabase().rawQuery("select * from recipientdata where Email='"+Email+"' AND Password='"+Password+"' AND status=1",null);
        while(cursor.moveToNext())
        {
            f=1;
        }
    ...
    public void insertintoitems(String itemtype,String itemdetails, int itemcount,String mobile)//linked with uploaditem to insert values
    {
        this.getWritableDatabase().execSQL("insert into itemsdata(itemtype,itemdetails,itemcount,mobile,BPL) values('"+itemtype+"','"+itemdetails+"','"+itemcount+"','"+mobile+"','0')");
    }
    ...
    {
        int f=0;
        Cursor cursor=this.getReadableDatabase().rawQuery("select * from ngodata where email='"+email+"' AND password='"+password+"' AND status=1",null);
        while(cursor.moveToNext())
        {
            f=1;
        }
    ...
    //Validations
    public int ngoidvalidation(String ngoid)
    {
        int f=0;
        Cursor cursor=this.getReadableDatabase().rawQuery("select * from ngodata where ngoid='"+ngoid+"'",
                null);
        while(cursor.moveToNext())
        {
            f=1;
    ...
    public int recbplvalidation(String bpl)
    {
        int f=0;
        Cursor cursor=this.getReadableDatabase().rawQuery("select * from recipientdata where BPL_No='"+bpl+"'",
                null);
        while(cursor.moveToNext())
        {
            f=1;
